...
export const GLOSSARY = {
  ...
  projected_standings: {
    ...
    body: "Each team's projected net rating and win total from its roster's impact, ranked within its conference. It models rotation: a fixed 19,680-minute budget goes to the best players first (up to each one's established minutes), so a trade is judged on minutes DISPLACEMENT — a star who benches a good player is worth less than one who displaces a scrub, and injury-prone players carry less weight because they bring fewer minutes. Minutes are availability-aware (a star who missed time to injury gets those minutes back), and a real-age aging curve nudges each projection up toward a ~26.5 peak and down after. With no moves staged it returns the current-roster baseline exactly, so the Δ you see is purely the effect of your trades and signings — did you make a team better or worse for next year. Fit R²=0.85 vs actual net rating; still a talent-on-hand projection, not a full-season forecast (no coaching or playoff translation).",
