...
import { TEAM_IDS, teamMeta, teamProjection } from "@/lib/league";
...
export default function StandingsPage() {
  ...
  const rows = TEAM_IDS.map((id) => ({ id, p: teamProjection(id, lg.contracts) }))
    .filter((r): r is { id: string; p: NonNullable<ReturnType<typeof teamProjection>> } => !!r.p)
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mb-5 text-sm leading-relaxed text-[var(--muted)]">
        Each team&rsquo;s projected record from its roster&rsquo;s minutes-weighted impact.
        {moved
      ...
      <div className="overflow-hidden rounded-lg border border-[var(--border)]">
        <div className="grid grid-cols-[1.5rem_1fr_4.5rem_3.5rem_3rem] items-center gap-2 border-b border-[var(--border)] bg-[var(--panel-2)] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--muted)]">
          <span>#</span>
          <span>Team</span>
          ...
          return (
            ...
            >
              ...
              <span className="flex min-w-0 items-center gap-2 font-semibold">
                ...
                <span className="truncate">{teamMeta(r.id).name}</span>
              </span>
      ...
      <p className="mt-3 text-[11px] text-[var(--muted)]">
        Wins map from projected net rating (≈ 40 + 2 wins per net-rating point); the roster-value → net-rating fit is R²=0.97 on 2025-26 teams. Record capped to a plausible NBA range.
      </p>
    </div>
  );
}
