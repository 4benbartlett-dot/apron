...
export const metadata: Metadata = {
  title: "NBA CBA Rules Coverage & Accuracy — 231 Tests Against the 2023 CBA | Over the Apron",
  description:
    "Exactly which 2023 CBA rules the trade machine enforces, what's approximate, and what isn't modeled — verified by 231 unit tests and real July 2026 move replays.",
};
...
const APPROXIMATE = [
  ...
  "Team strength projects a net rating and a win total for every team \u2014 ranked by conference on /standings, with team pages showing the record and the exact win swing your moves cause. It models ROTATION by POSITION: the 19,680-minute budget (5 \u00d7 48 \u00d7 82) is split into five on-court spots of 3,936 minutes each, handed to the best players first at their primary position, with versatile players sliding to a measured secondary spot. So a trade is judged on minutes DISPLACEMENT that respects position \u2014 acquiring a center for a team already anchored at center mostly benches one of them, while the same center fills an empty middle for a team that needs one; the projected rotation (with minutes) shows on every team page. Minutes are availability-aware: each player's projected role is his minutes/game over a durability-regressed games count (from real 2025-26 games played), so a star who missed time to injury gets those minutes back while an iron-man is trimmed to a sustainable load. A REAL-AGE aging curve (birth-year ages scraped for 100% of rated players) nudges each projection up toward a ~26.5 peak and down after, applied only to deltas so it never disturbs the baseline. Fit R\u00b2=0.82 vs the shipped baseline and 0.85 vs actual 2025-26 net rating; wins map from net rating (\u2248 40 + 2 wins per point). No-move rosters return the baseline exactly, so any delta is purely your moves \u2014 still a current-roster projection, not a full-season forecast",
  "Honest bounds on the impact metric itself: it\u2019s benchmarked against BPM and internal baselines, NOT against EPM, DARKO, LEBRON, RAPTOR, betting markets, or playoff outcomes \u2014 so \u201cbetter than BPM\u201d is the proven claim, not \u201cbest all-in-one model.\u201d Those all-in-one metrics live behind interactive apps with no public bulk export, so a head-to-head isn\u2019t something we can just download; and the 2026-27 market win totals that would anchor the team model aren\u2019t posted this early in the offseason. The RAPM history behind it spans 2021\u20132026 only (more seasons would sharpen the priors and mover validation); stint reconstruction still has a low-retention tail (median 48 lineup-minutes); and there is no role-change, coaching, or playoff-translation layer. Fit/synergy is a real but small signal we let explain and adjust, never dominate. Where this can become genuinely special is the combination \u2014 impact, minutes, availability, aging, cap cost, and team outcomes modeled together \u2014 not any single scalar",
  "Positions are data-driven: a player's primary spot is where he actually logged the most minutes (Basketball-Reference play-by-play position shares), and a genuine SECONDARY position is any spot he played \u226520% of his minutes at \u2014 so the ~1-in-5 versatile players (a combo guard, a switchable forward) can slide in the rotation model. Players the play-by-play table misses fall back to BRef's assigned position, then the transactions feed; deep-bench flexibility falls back to adjacent spots",
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        ...
        <p className="mt-1 text-sm text-[var(--muted)]">
          ...
          verified by 231 unit tests against the CBA's own text, and by replaying real 2026 free-agency moves through the engine. No hand-waving.
        </p>
        </div>
        <span className="stamp stamp-in mt-1 text-[13px] text-[var(--tier-below_cap)]">Audited · 231 tests</span>
      </div>
